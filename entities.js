...
function aStarPath(grid, startCol, startRow, endCol, endRow, features, roads) {
  if (startCol === endCol && startRow === endRow) return [];
  ...
  while (heap.length > 0) {
    ...
    for (const [dc, dr, cost] of dirs) {
      ...
      if (features) {
        ...
        if (feat === 1) moveCost *= 2.0;   // trees: expensive to path through
        if (feat === 2) moveCost *= 1.8;   // rocks: expensive
        if (feat === 3) moveCost *= 1.3;   // ruins: slightly expensive
      }
      if (roads && roads.has(nIdx)) moveCost *= 0.7; // roads: cheaper
